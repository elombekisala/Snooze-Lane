...
struct QuickDestinationEditView: View {
    ...
    init(
        quickDestinationsManager: QuickDestinationsManager,
        editingDestination: QuickDestination? = nil
    ) {
                ...
                VStack(spacing: 8) {
                    Text(
                        editingDestination != nil
                            ? "Edit Quick Destination" : "Add Quick Destination"
                    )
                    .font(.title2)
                    .fontWeight(.bold)
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            VStack(alignment: .leading, spacing: 4) {
                                ...
                                if let placemark = selectedMapItem.placemark {
                                    ...
                                    Text(
                                        "Lon: \(placemark.coordinate.longitude, specifier: "%.6f")"
                                    )
                                    .font(.caption)
                                    .foregroundColor(.white.opacity(0.7))
                                }
                ...
                VStack(spacing: 12) {
                    Button(action: {
                        ...
                        HStack {
                            ...
                            Text(
                                editingDestination != nil
                                    ? "Update Destination" : "Save Destination")
                        }
                        .font(.headline)
                        .foregroundColor(.white)
                ...
                Button(action: {
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        if let placemark = mapItem.placemark {
                            ...
                            Text(
                                "\(placemark.coordinate.latitude, specifier: "%.6f"), \(placemark.coordinate.longitude, specifier: "%.6f")"
                            )
                            .font(.caption)
                            .foregroundColor(.secondary)
                        }
