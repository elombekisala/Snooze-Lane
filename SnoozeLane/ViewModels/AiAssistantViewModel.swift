...
@Observable
class AiAssistantViewModel: NSObject, ObservableObject, AVAudioRecorderDelegate,
        ...
        do {
            ...
            animationTimer = Timer.scheduledTimer(
                ...
                block: { [unowned self] _ in
                    ...
                    let power = min(
                        1,
                        max(0, 1 - abs(Double(self.audioRecorder.averagePower(forChannel: 0)) / 50))
                    )
                    self.audioPower = power
                })

            recordingTimer = Timer.scheduledTimer(
                withTimeInterval: 1.6, repeats: true,
                block: { [unowned self] _ in
                    guard self.audioRecorder != nil else { return }
                    self.audioRecorder.updateMeters()
                    let power = min(
                    ...
                    if let prevAudioPower = self.prevAudioPower,
                        prevAudioPower < 0.25 && power < 0.175
                    {
    ...
    func playAudio(data: Data) throws {
        ...
        animationTimer = Timer.scheduledTimer(
            ...
            block: { [unowned self] _ in
                ...
                let power = min(
                    1, max(0, 1 - abs(Double(self.audioPlayer.averagePower(forChannel: 0)) / 160)))
                self.audioPower = power
            })
    }
