...
@Observable final class QuickDestinationsManager {
        ...
        listener = db.collection("users").document(userId).collection("quickDestinations")
            .addSnapshotListener { [weak self] snapshot, error in
                guard let self = self, let snapshot = snapshot else {
                    print("Error fetching quick destinations: \(error?.localizedDescription ?? "Unknown error")")
                    return
                }
                ...
                for document in snapshot.documents {
                    ...
                    if let destination = self.quickDestinationFromFirebaseData(data, id: document.documentID) {
    ...
    private func quickDestinationFromFirebaseData(_ data: [String: Any], id: String) -> QuickDestination? {
        guard let title = data["title"] as? String,
              ...
              let category = QuickDestinationCategory(rawValue: categoryRaw) else {
            return nil
        }
    ...
    private func saveQuickDestinationToFirebase(_ destination: QuickDestination, userId: String) async throws {
    ...
    private func removeQuickDestinationFromFirebase(_ destinationId: String, userId: String) async throws {
        try await db.collection("users").document(userId).collection("quickDestinations")
    ...
    private func syncAllToFirebase(_ userId: String) async throws {
        ...
        let snapshot = try await db.collection("users").document(userId).collection("quickDestinations").getDocuments()
        for document in snapshot.documents {
